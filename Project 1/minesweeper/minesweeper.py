    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] -1, cell[0]+2):
            for j in range(cell[1]-1, cell[1]+2):
                if (i, j) in self.mines:
                    mineCount +=1
                if 0 <= i <self.height and 0<=j < self.width and (i,j) not in self.safes and (i, j) not in self.mines and (i, j) not in self.moves_made:
        ...
        for sentence in self.knowledge:
            if newSentence.cells.issubset(sentence.cells) and sentence.count > 0 and newSentence.count > 0 and newSentence != sentence:
                ...
                newSentenceSubset = Sentence(list(newSubset), sentence.count - newSubset.count)
